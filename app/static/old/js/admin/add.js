...
function createSchoolClassOption(school_plan_name){
...
function createSchoolClassesOptions(){
    select_school_class.innerHTML = "";
    Object.keys(plans).forEach(school_plan_name => {
        createSchoolClassOption(school_plan_name)
    });
}
...
function createLessons(weekday){
...
function getWeekNumbersDifference(date1, date2) {
    return Math.abs(getWeekNumber(date1) - getWeekNumber(date2));
}

function setDateLabelText(date, weekday) {
    ...
    document.querySelector("#date_label").innerHTML = str;
}
